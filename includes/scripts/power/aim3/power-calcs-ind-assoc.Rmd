...
# read in snps from appendix a
# at ~\Documents\dissertation\unc-dissertation-markdown\includes\table-data

appa.snps = read_excel("~/dissertation/unc-dissertation-markdown/includes/table-data/Copy of Full_list_lipids_known_loci_02102017-annotated.xlsx")
#head(appa.snps)
...
# 3. then save as .csv file to following location
maf.amr = read.table("~/dissertation/unc-dissertation-markdown/includes/scripts/power/aim3/sps-download.csv", sep=";", header=T, skip=1)
head(maf.amr)
```
...
kable(maf.amr[maf.amr$population=="Population Set 1", select.c],
      row.names = F)
```
...
b.snps = appa.snps[,c("rsid", "EAF", "Effect allele", "beta")]
#colnames(b.snps)

# redo amr data
#head(maf.amr)
maf.amr2 = maf.amr[maf.amr$population=="Population Set 1",
                   c("SNP", "MAF", "reference")]
...
#head(maf)
colnames(maf) = c("rsid", "EAF.appa", "allele.appa", "beta",
                  "maf.amr", "allele.amr")
...
sub=maf[c("rsid", 'beta', 'EAF.appa', 'maf.amr', 'pwr.hdl', 'pwr.ldl', 'pwr.tg')]
sub
...
### HDL-related SNPS
```{r, results='markup'}
# any power estimates exceeding 0.8 (by trait specific effect)?
...
table.sig.snp = appa.snps[(appa.snps$rsid %in% snp.sig$rsid), c("Trait", "rsid", "Author", "Chr", 
                                                    "Gene   ", "Effect allele", "beta")]
write.csv(table.sig.snp, file="~/dissertation/unc-dissertation-markdown/includes/scripts/power/aim3/sig-snp.csv",
          row.names = F)
kable(table.sig.snp)
```
