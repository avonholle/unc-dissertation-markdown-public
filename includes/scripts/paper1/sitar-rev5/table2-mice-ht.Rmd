---
title: "Table 2, height trajectories -- SITAR parameter estimates. Uses full infancy cohort in preventive trial."
...
```{r, child='../table3-data-handle-weight-impute-rev.Rmd', eval=F}
...
#              dat.m = dat.m[complete.cases(dat.m),] # did not impute the outcome or sex variable so exclude those. 
              ...
              dat.m$graffar.index = dat.m$v368c
              return(dat.m) 
...
nrow(imp.data2[[2]])
length(unique(long1.orig2$id)) # 602
summary(long1.orig2)
...
#  x = temp.dat
df1$log.value = log(df1$value.ht)
...
# analyses, based on fit statistics from table3-w-fcns.pdf
# best fit was beta and gamma, sex-specific curve
...
sim.ht.r5.strat = func.5.strat(df1)

      # 2. extract out random effects ----------------------
      
      re.vals = random.effects(sim.ht.r5.strat)
      re.vals.df = data.frame(re.vals)
      re.vals.df$id = rownames(re.vals.df)
      colnames(re.vals.df)
      head(re.vals.df)
      nrow(re.vals.df) # 1412
      summary(re.vals.df)
  
      # read in data
      setwd("~/")

      save(re.vals.df, file="re-ht-rev.Rdata") # save this so I don't have to re-run when debugging. takes  a long time.
```
...
load("re-ht-rev.Rdata") # has re.vals.df, list of imputation data frames
nrow(re.vals.df) # 1412
```
...
```{r}
# CHECK 
# CHECK if this result matches the SITAR package, especially for velocity.
# using code from ~\GitHub\unc-dissertation-markdown-p2\includes\scripts\paper1\longleaf\model-fit\table3-w-fcns.Rmd

 names(df1)
 nrow(df1) # 8472
 df1.cc = df1[complete.cases(df1),]
 nrow(df1.cc) # 5894
...
func.5 = function(df){
  ...
  temp.dat.g = groupedData(log.value ~ time2 | id, data=df) # set up grouped data object for plotting
    ...
    sim.wt.r5 <-
      nlme(log.value ~ fitnlme.f.r5(time2,s1,s2,s3,s4,alpha0, beta0, beta1),
      data=df1.cc, na.action=na.omit, # do complete case
 ...
 sim.wt.r5 = func.5(df1.cc)
 summary(sim.wt.r5)
 orig.re = data.frame(ranef(sim.wt.r5))
 class(orig.re)
 dim(orig.re) # 1050 by 2
 summary(ranef(sim.wt.r5))
 
# check with sitar package
head(df1.cc)

  temp.dat.g = groupedData(log.value ~ time2 | id, data=df1.cc) # set up grouped data object for plotting
  
 m.r5 = sitar(x=time2, y=log.value,
              data=df1.cc, id=factor(id),
              knots=myknots, fixed=c('a'), random='b+c')

 summary(m.r5)
 sitar.re = data.frame(ranef(m.r5))
 dim(sitar.re) # 1050 by 2
 length(unique(df1.cc$id)) # 1050
 class(sitar.re)
 summary(ranef(m.r5))
 ...
 # lastly, check association between the ht random effects and HDL-C.
 # year 1 cohort data
...
# combine re from both models
names(sitar.re)
names(orig.re)

re = data.frame(id = unique(df1.cc$id), 
                b.sitar = sitar.re$b,
                c.sitar = sitar.re$c,
                b.orig = orig.re$beta0,
                c.orig = orig.re$beta1)
dim(re)
names(re)
 
lipids = merge1[c("ID", "LDLChlstrl", "HDLchlstrl")]
colnames(lipids) = c("id", "ldl", "hdl")

all.dat = merge(lipids,
                re,
                by="id",
                all.x=T)
nrow(all.dat) # 602
...
lm(data=all.dat, hdl ~ c.sitar)

lm(data=all.dat, hdl ~ c.orig)
...
save(list.0, list.1, file="table2-ht-rev.Rdata")
...
setwd("~/GitHub/unc-dissertation-markdown-p2/includes/scripts/paper1")
#setwd("~/dissertation/unc-dissertation-markdown-p2/includes/scripts/paper1")
load("table2-ht-rev.Rdata") # load the list.0 and list.1 data frame
```
...
# df2.ad = lapply(list.1, function(y) y[[2]]) # extract out 2nd element of each model, nested within 
# df2.un = lapply(list.2, function(y) y[[1]]) # for second approach only in s1 function above.

# df2.ad = do.call(rbind.data.frame, df2.ad) # now combine all data frames still in a nested list into one data frame
# 
# df2.un = do.call(rbind.data.frame, df2.un)
# 
# df2 = rbind.fill(df2.un, df2.ad)
# head(df2)
# table(df2$covs)
...
#df$exposure = sapply(strsplit(as.character(df$form)," +"), "[", 1)
...
both.df. = rbind.fill(df.ad., df.unad.)
...
tablelasso.bysex = dcast(both.df[both.df$sex %in% c("male", "female"),],
                       cov.names.f ~ sex + covs + outcome, 
                    value.var='lasso.coef')
tablelasso.bysex
...
save(tabledat.all, file="tableht.Rdata") # output to tables-ms.Rmd
...
#df2.2[df2.2$cov.names.f=="Graffar Index" & df2.2$sex=="both" & df2.2$outcome=="beta1" &
#        df2.2$covs=="adj",]
...
df2.2$outcome.f = factor(df2.2$outcome, labels=c("Tempo", "Velocity"))
levels(df2.2$outcome.f)

#df2.2[df2.2$covs=="adj", c("outcome", "sex", "cov.names.f2", "coef", "lci", "uci")]

beta.height = ggplot(df2.2[df2.2$covs=="adj",], 
...
png(file="aim1-fig1-beta-height.png", height=6, width=6, units='in', res=300)
  beta.height +
