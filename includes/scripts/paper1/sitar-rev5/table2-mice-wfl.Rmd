---
title: "Table 2, WFL trajectories -- SITAR parameter estimates. Use mice for estimates. Uses full infancy cohort in preventive trial."
...
# best fit was alpha and beta , sex-specific curve
...
#  x = temp.dat
df1$log.value = log(df1$value.wfl)
...
sim.wfl.r6.strat = func.6.strat(df1)

      # 2. extract out random effects ----------------------
      
      re.vals = random.effects(sim.wfl.r6.strat)
  ...
  summary(re.vals.df)
      ...
      save(re.vals.df, file="re-wfl-rev.Rdata") # save this so I don't have to re-run when debugging. takes  a long time.
```
...
load("re-wfl-rev.Rdata") # has re.vals.df, list of imputation data frames
nrow(re.vals.df) # 1412
```
...
colnames(re.vals.merge)[2] = "alpha0"
head(colnames(re.vals.merge))
nrow(re.vals.merge)
class(re.vals.merge)
dim(re.vals.merge)
head(re.vals.merge)
...
elastic = function(j, o, i) {
  ...
  # j = "female"; o="beta0"; i="all" ; # for debugging
...
save(list.0, list.1, file="table2-wfl-rev.Rdata")
...
setwd("~/GitHub/unc-dissertation-markdown-p2/includes/scripts/paper1")
#setwd("~/dissertation/unc-dissertation-markdown-p2/includes/scripts/paper1")
load("table2-wfl-rev.Rdata") # load the list.0 and list.1 data frame
```
...
df.ad = do.call(rbind.fill, list.0)
...
levels(factor(df.ad$cov.names))
levels(factor(df.ad$cov.names.f))
...
save(tabledat.all, file="tablewfl.Rdata") # output to tables-ms.Rmd
...
#      format="latex",
      ...
      col.names =  c("Characteristic", 
                     rep(c("Size", "Tempo"), 4)),
      caption='WFL outcome association with SITAR parameters, stratified by gender') %>%
...
#  kable_styling(latex_options = "striped", "scale_down", full_width=F) %>%
  ...
  column_spec(column=8, width = "10em") %>%
  column_spec(column=9, width = "10em") %>%
  add_footnote(c("Bold values indicate significance at FDR of 0.05"),
               notation = "symbol")
...
df2.2[df2.2$outcome.f=="Tempo" & df2.2$covs=="adj" & df2.2$cov.names.f=="Mother's total yr educ",]
...
levels(factor(df2.2$cov.names.f2)) # check

levels(df2.2$outcome)
df2.2$outcome.f = factor(df2.2$outcome, labels=c("Size", "Tempo"))
levels(df2.2$outcome.f)

#df2.2[df2.2$covs=="adj", c("outcome", "sex", "cov.names.f2", "coef", "lci", "uci")]

alpha0.wfl = ggplot(df2.2[df2.2$covs=="adj" & df2.2$outcome=="alpha0",], 
...
# put beta0 on different plot as scale is different...
# -------------------------------------------

beta.wfl = ggplot(df2.2[df2.2$covs=="adj" & df2.2$outcome == "beta0",], 
       aes(x=outcome.f, y=coef, group=sex, colour=sex)) +
  facet_grid(cov.names.f2 ~ outcome.f, scale='free') +# label_parsed allows greek symbols in the strip text
    geom_errorbar(aes(ymin=lci, ymax=uci), width=.1, position=pd, size=1.5) +
...
NOTE: Small black dots are the lasso parameter estimates.
...
png(file="aim1-fig1-alpha-wfl.png", height=6, width=6, units='in', res=300)
  alpha0.wfl +
    theme(legend.position = "right")
dev.off()

png(file="aim1-fig1-beta-wfl.png", height=6, width=6, units='in', res=300)
  beta.wfl +
    theme(legend.position = "right")
dev.off()
...
#      format="latex",
      format="html",
      col.names =  c("Characteristic", 
                     c( "Size", "Tempo", "Size", "Tempo")),
      caption='WFL outcome association with SITAR parameters, pooled across sex of child') %>%
  kable_styling(bootstrap_options = c("striped")) %>%
#  kable_styling(latex_options = "striped", "scale_down", full_width=F) %>%
